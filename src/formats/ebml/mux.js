...
const getint16bytes = function(value) {
...
const encodeBlocks = function(frames, clusterTimestamp) {
  return frames.reduce((acc, frame) => {
    ...
    const blockData = concatTypedArrays(
      ...
      getint16bytes(frame.timestamp - clusterTimestamp),
      [flagByte],
      frame.data
    );
...
const encodeClusters = function(frames) {
  const clusters = [];
  const keyframes = [];
  ...
  keyframes.forEach(function(startIndex, i) {
    const endIndex = keyframes[i + 1] ? keyframes[i + 1] : frames.length;
    const clusterTimestamp = frames[startIndex].timestamp;

    clusters.push([T.Cluster, [
      [T.ClusterTimestamp, getint16bytes(frames[startIndex].timestamp)]
    ].concat(encodeBlocks(frames.slice(startIndex, endIndex), clusterTimestamp))]);
  });

  return clusters;
};

const generateEBML = function({tracks, frames, cues, info}) {
  ...
  const segment = [T.Segment, [
    [T.SegmentInformation, [
      ...
      [T.SegmentDuration, info.duration],
      [T.MuxingApp, 'transcodejs'],
      [T.WritingApp, 'transcodejs']
    ]],
    [T.Tracks, ebmltracks]
  ].concat(encodeClusters(frames))];
